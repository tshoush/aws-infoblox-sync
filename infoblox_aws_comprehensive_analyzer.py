...
def load_aws_data(csv_file: str) -> List[Dict]:
    ...
    try:
        ...
        for _, row in df.iterrows():
            network = {
                'cidr': row.get('cidr', row.get('CIDR', row.get('network'))),
                'tags': {}
            }
            ...
            if 'tags' in row and pd.notna(row['tags']):
                try:
                    network['tags'] = json.loads(row['tags'])
                except:
                    network['tags'] = eval(row['tags']) if row['tags'] else {}
            
            # Add other columns as tags
            for col in df.columns:
                if col not in ['cidr', 'CIDR', 'network', 'tags'] and pd.notna(row[col]):
