...
# --- BEGIN LOG FILE TEST ---
LOG_FILE_PATH = 'aws_infoblox_vpc_manager.log'
try:
    with open(LOG_FILE_PATH, 'a') as f:
        f.write(f"--- Log file test write at {datetime.now()} ---\n")
    print(f"INFO: Successfully wrote a test line to '{LOG_FILE_PATH}'.")
except Exception as e:
    print(f"ERROR: Could not write to log file path '{LOG_FILE_PATH}'. Error: {e}")
# --- END LOG FILE TEST ---
    ...
    try:
        ...
        if args.create_missing and comparison_results['missing']:
            ...
            if args.dry_run:
                ...
                # ea_analysis['ea_results'] is like {'ea_name': 'created' or 'exists'}
            ...
            if eas_to_report:
                ...
                # Optionally print a message if no EAs for the report
                # print(f"   ℹ️ No EAs for the {report_ea_title.lower().replace(':', '')} report.")
...
class InfoBloxClient:
    ...
    def get_network_by_cidr(self, cidr: str, network_view: str = "default") -> Optional[Dict]:
        ...
        # ---- START DEBUG PRINT ----
        print(f"DEBUG: InfoBloxClient.get_network_by_cidr called with cidr='{cidr}', network_view='{network_view}' (length: {len(network_view)})")
        # ---- END DEBUG PRINT ----
        params = {
            'network': cidr,
            'network_view': network_view,
    ...
    def get_network_container_by_cidr(self, cidr: str, network_view: str = "default") -> Optional[Dict]:
        ...
        # ---- START DEBUG PRINT ----
        print(f"DEBUG: InfoBloxClient.get_network_container_by_cidr called with cidr='{cidr}', network_view='{network_view}' (length: {len(network_view)})")
        # ---- END DEBUG PRINT ----
        params = {
            'network': cidr,
            'network_view': network_view,
...
class VPCManager:
    ...
    def create_missing_networks(self, missing_networks: List[Dict], network_view: str = "default", 
            ...
            try:
                ...
                if dry_run:
                    ...
                    if existence_check['exists'] and existence_check['type'] == 'container':
                        ...
                        results.append({
                            'cidr': cidr,
                            'vpc': item['vpc'],  # Include full vpc item for reporting
                            'action': 'would_reject',
        ...
        # Generate rejected networks CSV if there are any rejections (actual run)
        if rejected_networks and not dry_run:
            # The 'rejected_networks' list already contains dicts with 'vpc', 'reason', 'error_details'
            self._generate_rejected_networks_csv(rejected_networks, is_dry_run_issues=False)
        elif dry_run:
            # For dry run, compile a list of issues from the 'results' list
            dry_run_issues_to_report = []
            for res_item in results:
                # Only report on items that wouldn't be created or would be problematic
                if res_item.get('action') == 'would_reject':
                    dry_run_issues_to_report.append({
                        'vpc': res_item['vpc'],  # This was added above
                        'reason': res_item.get('reason', 'N/A'),
                        'error_details': res_item.get('details', 'N/A') # Or a more specific dry_run_reason
                    })
                # Add other 'would_fail' conditions here if they are simulated in dry_run
                # For now, 'would_reject' is the primary dry-run issue type we capture with details.
            
            if dry_run_issues_to_report:
                self._generate_rejected_networks_csv(dry_run_issues_to_report, is_dry_run_issues=True)
            else:
                # Optionally, inform that no dry-run issues were found for the report
                logger.info("No specific issues (like 'would_reject') found during dry run to report in CSV.")
                # Consider if a report should still be generated saying "No issues found" or skip it.
                # For now, skipping if empty. The _generate_rejected_networks_csv also has a check.
        
        return results
    
    def _generate_rejected_networks_csv(self, data_to_report: List[Dict], base_filename: str, is_dry_run_issues: bool = False) -> None:
        """Generate CSV report for rejected networks or dry-run issues."""
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        # Construct filename, e.g., reports/dry_run_vpc_issues_20250604_150000.csv
        # or reports/rejected_networks_20250604_150000.csv
        prefix = "dry_run_vpc_creation_issues" if is_dry_run_issues else "rejected_networks"
        filename_with_ts = f"{prefix}_{timestamp}.csv"
        full_path = os.path.join(reports_dir, filename_with_ts)
        
        csv_data = []
        for item in data_to_report:
            vpc = item['vpc'] # Expect 'vpc' to be a dictionary with VPC details
            csv_row = {
                'AccountId': vpc.get('AccountId', 'N/A'),
                'Region': vpc.get('Region', 'N/A'),
                'VpcId': vpc.get('VpcId', 'N/A'),
                'Name': vpc.get('Name', 'N/A'),
                'CidrBlock': vpc.get('CidrBlock', 'N/A'),
                'Reason': item.get('reason', 'N/A'),
                'Details': item.get('error_details', item.get('error', 'N/A')) # 'error' for dry run 'would_fail'
            }
            csv_data.append(csv_row)
        
        if not csv_data:
            logger.info(f"No data to write for {prefix} report.")
            # Optionally print a message that no issues were found for this report type
            # print(f"\nℹ️ No items for {prefix.replace('_', ' ')} report.")
            return

        df = pd.DataFrame(csv_data)
        df.to_csv(full_path, index=False)
        
        report_type_msg = "Dry-run VPC creation issues" if is_dry_run_issues else "Rejected networks"
        logger.info(f"Generated {report_type_msg} CSV: {full_path}")
        print(f"\n📄 {report_type_msg.upper()} FILE GENERATED: {full_path}")
...
def show_and_edit_config():
    ...
    print(f"👤 Username: {config['INFOBLOX_USERNAME'] or '(not set)'}")
...
def get_config(skip_network_view_prompt=False, config_override=None):
    ...
    if config_override:
        ...
        username = config_override['INFOBLOX_USERNAME']
